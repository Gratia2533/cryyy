...
"""
選擇具有較小的boundary distance，|confidence-Topt|
"""
####參數設定####
...
parser.add_argument('--evaluation_dir', type=str, help='Path to the evaluation directory')  # 新增命令行參數
args = parser.parse_args()

iter = args.iter
evaluation_dir = args.evaluation_dir  # 根據autoAL_basePts.py的evaluation_path做設定
...
html_dir = evaluation_dir + iter + '_evaluation.html'
topt_log_dir = evaluation_dir + 'topt_log.txt'
    ...
    for line in lines:
        ...
        if len(values) >= 9:  # 確保有足夠的值
            try:
                ...
                boundary_distance = abs(confidence-topt)#計算|confidence-topt|
                modified_line = f"{values[0]} {values[1]} {values[3]} {values[4]} {boundary_distance}\n"
...
def is_match(line_gt, line_box):
    ...
    radius_gt = gt_values[2] / 2  
    
    # 預測框的圓心座標 = 左上角座標 + 寬高的一半
    center_box_x = box_values[0] + box_values[2] / 2
    center_box_y = box_values[1] + box_values[3] / 2
    radius_box = (box_values[2] * box_values[3]) ** 0.5 / 2  # 使用幾何平均計算半徑
    
    # 計算圓形的IOU
    iou = calculate_circle_iou([center_gt_x, center_gt_y, radius_gt], 
                               [center_box_x, center_box_y, radius_box])
...
if data_frames:  # 檢查是否有任何非空檔案
    ...
    # 按照每一行第4個元素由小到大排序(與boundary距離越小越不確定)
