...
"""
將confidence正規化到0-1之間，才進行entropy score
"""
####參數設定####
...
# Step 2: 處理每個檔案，正規化 confidence 並計算 entropy score
def process_cbox_file(input_file, output_file, min_confidence, max_confidence):
    ...
    for line in lines:
        ...
        if len(values) >= 9:  # 確保有足夠的值
            try:
                confidence = float(values[8])
                # 正規化 confidence
                normalized_confidence = normalize_confidence(confidence, min_confidence, max_confidence)
                # 計算 entropy score
                entropyScore = entropy_score(normalized_confidence)
                # 保存結果
...
# 找出所有檔案中的 confidence 的最小值和最大值
min_confidence, max_confidence = find_confidence_range(CBOX_dir)
...
for filename in os.listdir(CBOX_dir):
    if filename.endswith(".cbox"):
        ...
        process_cbox_file(input_file_path, output_file_path, min_confidence, max_confidence)
